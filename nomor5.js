// 5. membuat sebuah function yang berfungsi membantu Ibu Daniela untuk mendapatkan informasi berupa Total Keuntungan, Total Modal, Produk Buku Terlaris, Penulis Buku Terlaris dan Persentase Keuntungan dari data penjualan yang telah disediakan
...
function getInfoPenjualan(dataPenjualan) {
  ...
  const totalModal = dataPenjualan
    ...
    .reduce((acc, curr) => acc + curr, 0); // ACC = Akumulator || CURR = Current
  ...
  const novelTerlaris = dataPenjualan
    .filter(
      (a) =>
        a.totalTerjual === Math.max(...dataPenjualan.map((b) => b.totalTerjual))
    )
    .map((a) => a.namaProduct)
    .toString();
  ...
  const stokTerkecil = dataPenjualan.reduce(
